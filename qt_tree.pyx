...
cdef DTYPE_t get_max_for_poincare_rect(DTYPE_t[2] min_bounds, DTYPE_t[2] max_bounds) nogil:
    return max(distance(min_bounds[X_ax], min_bounds[Y_ax], max_bounds[X_ax], max_bounds[Y_ax]),
               distance(min_bounds[X_ax], min_bounds[Y_ax], min_bounds[X_ax], max_bounds[Y_ax]),
               distance(min_bounds[X_ax], min_bounds[Y_ax], max_bounds[X_ax], min_bounds[Y_ax]),
               distance(max_bounds[X_ax], max_bounds[Y_ax], min_bounds[X_ax], max_bounds[Y_ax]),
               distance(max_bounds[X_ax], max_bounds[Y_ax], max_bounds[X_ax], min_bounds[Y_ax]),
               distance(min_bounds[X_ax], max_bounds[Y_ax], max_bounds[X_ax], min_bounds[Y_ax])
               )
...
cdef class _QuadTree:
    ...
    cdef int insert_point(self, DTYPE_t[2] point, SIZE_t point_index,
        ...
        if cell.cumulative_size == 0:
            ...
            poincare_point[0] = point[X_ax]
            poincare_point[1] = point[Y_ax]
        ...
        # in it.
        poincare_point[X_ax] = cell.barycenter[0]
        poincare_point[Y_ax] = cell.barycenter[1]
    ...
    cdef SIZE_t _insert_point_in_new_child(self, DTYPE_t[2] point, Cell* cell,
        ...
        child.center[X_ax] = (child.min_bounds[X_ax] + child.max_bounds[X_ax]) / 2.
        child.center[Y_ax] = (child.min_bounds[Y_ax] + child.max_bounds[Y_ax]) / 2.

        child.barycenter[X_ax] = point[X_ax]
        child.barycenter[Y_ax] = point[Y_ax]
        ...
        width = get_max_for_poincare_rect(child.min_bounds, child.max_bounds)
    ...
    cdef bint _is_duplicate(self, DTYPE_t[2] point1, DTYPE_t[2] point2) nogil:
        ...
        res &= fabs((point1[0]) - point2[0]) <= EPSILON
        res &= fabs((point1[1]) - point2[1]) <= EPSILON
        return res
    ...
    cdef void _init_root(self, DTYPE_t[2] min_bounds, DTYPE_t[2] max_bounds) nogil:
        ...
        root.center[X_ax] = (max_bounds[X_ax] + min_bounds[X_ax]) / 2.
        root.center[Y_ax] = (max_bounds[Y_ax] + min_bounds[Y_ax]) / 2.

        width = get_max_for_poincare_rect(min_bounds, max_bounds)
